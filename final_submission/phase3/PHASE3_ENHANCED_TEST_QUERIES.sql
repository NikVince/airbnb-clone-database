...
-- Tables: bookings, guest_profiles, users, properties, host_profiles, booking_status
SELECT 
    b.booking_id,
...
FROM bookings b
    ...
    INNER JOIN host_profiles hp ON p.host_id = hp.host_profile_id
    INNER JOIN users hu ON hp.user_id = hu.user_id
    INNER JOIN booking_status bs ON b.booking_status_id = bs.status_id
...
-- Multi-table JOIN across 5 tables with aggregation
-- ==============================================
-- Business Purpose: Analyze property performance with bookings and reviews
-- Tables: properties, host_profiles, users, bookings, reviews, booking_status
SELECT 
    p.property_id,
    p.title AS property_title,
...
FROM properties p
    ...
    LEFT JOIN booking_status bs ON b.booking_status_id = bs.status_id
    LEFT JOIN reviews r ON b.booking_id = r.booking_id
WHERE bs.status_name = 'confirmed' OR bs.status_name IS NULL
GROUP BY p.property_id, p.title, u.first_name, u.last_name
...
-- Multi-table JOIN across 5 tables with window functions
-- ==============================================
-- Business Purpose: Analyze guest booking patterns with ranking
-- Tables: users, guest_profiles, bookings, properties, booking_status
SELECT 
    gu.user_id,
    CONCAT(gu.first_name, ' ', gu.last_name) AS guest_name,
    COUNT(DISTINCT b.booking_id) AS total_bookings,
...
-- Tables: users, host_profiles, properties, bookings, reviews, booking_status
SELECT 
    hu.user_id,
    CONCAT(hu.first_name, ' ', hu.last_name) AS host_name,
...
FROM users hu
    ...
    INNER JOIN properties p ON hp.host_profile_id = p.host_id
    LEFT JOIN bookings b ON p.property_id = b.property_id
    LEFT JOIN booking_status bs ON b.booking_status_id = bs.status_id
    LEFT JOIN reviews r ON b.booking_id = r.booking_id
WHERE bs.status_name = 'confirmed' OR bs.status_name IS NULL
...
-- Multi-table JOIN across 6 tables with subquery
-- ==============================================
-- Business Purpose: Analyze properties by location with market metrics
-- Tables: countries, cities, addresses, properties, bookings, reviews, booking_status, property_pricing
SELECT 
    co.country_name,
    c.city_name,
    COUNT(DISTINCT p.property_id) AS total_properties,
    AVG(pp.base_price_per_night) AS avg_price_per_night,
    AVG(r.rating) AS avg_rating,
    SUM(b.total_amount) AS total_revenue
FROM countries co
...
-- Tables: conversations, users, messages, properties, bookings
SELECT 
    c.conversation_id,
    CONCAT(gu.first_name, ' ', gu.last_name) AS guest_name,
    CONCAT(hu.first_name, ' ', hu.last_name) AS host_name,
...
FROM conversations c
    INNER JOIN users gu ON c.participant1_id = gu.user_id
    INNER JOIN users hu ON c.participant2_id = hu.user_id
    INNER JOIN bookings b ON c.booking_id = b.booking_id
    INNER JOIN properties p ON b.property_id = p.property_id
    LEFT JOIN messages m ON c.conversation_id = m.conversation_id
GROUP BY c.conversation_id, gu.first_name, gu.last_name, hu.first_name, hu.last_name, p.title
...
-- Tables: property_amenities, property_amenity_links, properties, reviews
SELECT 
    pa.amenity_name,
    pa.amenity_category,
    COUNT(DISTINCT pal.property_id) AS properties_with_amenity,
    AVG(r.rating) AS avg_rating,
    COUNT(DISTINCT r.review_id) AS total_reviews
FROM property_amenities pa
    INNER JOIN property_amenity_links pal ON pa.amenity_id = pal.amenity_id
    INNER JOIN properties p ON pal.property_id = p.property_id
    LEFT JOIN bookings b ON p.property_id = b.property_id
    LEFT JOIN reviews r ON b.booking_id = r.booking_id
GROUP BY pa.amenity_id, pa.amenity_name, pa.amenity_category
